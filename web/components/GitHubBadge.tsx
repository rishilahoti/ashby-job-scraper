...
/** Compact icon + live star count, all one link — for use on dark marketing surfaces. */
export function GitHubStarBadge({ className }: { className?: string }) {
  ...
  return (
    <a
      ...
      className={className}
    >
      <GitHubIcon className="w-4.5 h-4.5 sm:w-5 sm:h-5" />
      <span aria-hidden className="text-[#F5A623]">★</span>
      <span className="tabular-nums">{stars !== null ? stars : "—"}</span>
    </a>
  );
}
